...
public class OpenApiMergeService {
    ...
    public Map<String, Object> merged() {
        ...
        List<Src> sources = List.of(
                new Src("gateway", "http://localhost:" + serverPort + "/v3/api-docs", null),
                new Src("product", "http://localhost:" + serverPort + "/openapi/product", "openapi/product-openapi-fallback.json"),
                new Src("order",   "http://localhost:" + serverPort + "/openapi/order",   "openapi/order-openapi-fallback.json"),
                new Src("user",    "http://localhost:" + serverPort + "/openapi/user",    "openapi/user-openapi-fallback.json")
        );

        for (Src s : sources) {
            Map<String, Object> in = fetchOrFallback(s.url(), s.fallback(), s.name());
            if (!in.isEmpty()) mergeInto(out, in, s.name());
        }
        return out;
    }

    private record Src(String name, String url, String fallback) {}

    private Map<String, Object> fetchOrFallback(String url, String fallbackClasspath, String name) {
        try {
            ...
            // Fallback nga classpath nëse dështoi remote
            if (fallbackClasspath == null) return Collections.emptyMap();
            try {
                var res = new ClassPathResource(fallbackClasspath);
    ...
    private void mergeInto(Map<String, Object> out, Map<String, Object> in, String sourceName) {
        ...
        for (var e : inPaths.entrySet()) {
            ...
            String newPath = switch (sourceName) {
                ...
                default        -> originalPath; // gateway paths pa prefix
            };
        ...
        for (String section : List.of("schemas","parameters","responses","requestBodies","headers","securitySchemes","examples")) {
            ...
            for (var ent : inSec.entrySet()) {
                ...
                outSec.put(outSec.containsKey(key) ? key + "_svc_" + sourceName : key, val);
            }
        }
    }
}
